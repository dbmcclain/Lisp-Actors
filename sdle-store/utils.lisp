...
#+(or sbcl cmu openmcl)
  (:method ((object t) (class structure-class))
   (class-slots class))
#+allegro  
  (:method ((object t) (class clos::structure-class))
   (class-slots class))
#+sbcl
...
(defgeneric get-slot-details (slot-definition)
  ...
  #+allegro
  (:method ((slot-definition excl::structure-slot-definition))
   (list :name (slot-definition-name slot-definition)
         ;; :allocation (slot-definition-allocation slot-definition)
         ;; :initargs (slot-definition-initargs slot-definition)
         ;; :initform. dont use initform until we can
...
(defun store-32-bit (obj stream)
  "Write OBJ down STREAM as a 32 bit integer in network standard order. (bigendian)"
  (declare (optimize speed (debug 0) (safety 0))
           (type ub32 obj))
    (write-byte (ldb (byte 8 24) obj) stream)
...
(defun read-32-bit (buf)
  ...
  (let ((byte1 (read-byte buf))
    ...
    (make-ub32 byte1 byte2 byte3 byte4)))

#| ;; not used
(defun kwd (name)
  (values (um:intern-symbol (string name) :keyword)))
|#
