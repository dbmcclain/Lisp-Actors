...
(in-package #:stactors/network)

(eval-when (:compile-toplevel :load-toplevel :execute)
  (shadowing-import '(stactors/security:random))
  (import '(um:if-let
            um:when-let
            um:dlambda
            um:dlambda*
            ...
            stactors/security:secure-encoding
            stactors/security:secure-decoding
            stactors/security:byte-decode-obj
            stactors/security:crypto
            stactors/security:make-u8-vector
            stactors/security:convert-vector-to-integer
            stactors/security:+MAX-FRAGMENT-SIZE+
            stactors/security:assemble-sks
            stactors/security:time-to-renegotiate?
            
            stactors/bridge:bridge-register
            stactors/bridge:bridge-unregister
            stactors/bridge:bridge-handle-reply
            stactors/bridge:bridge-reset
            stactors/bridge:bridge-deliver-message

            stactors/lfm:ensure-system-logger
            stactors/lfm:kill-system-logger
            
            stactors/base:assemble-ask-message

            scatter-vec:scatter-vector
            scatter-vec:add-fragment

            #-:USING-ECC-CRYPTO stactors/srp6-rsa:server-negotiate-security-rsa
            #-:USING-ECC-CRYPTO stactors/srp6-rsa:client-negotiate-security-rsa

            #+:USING-ECC-CRYPTO stactors/srp6-ecc:client-negotiate-security-ecc
            #+:USING-ECC-CRYPTO stactors/srp6-ecc:server-negotiate-security-ecc
            )))

;; -----------------------------------------------------------------------

(defvar *default-port*            65002)
...
(defun resched-random-timer (timer)
    ...
    (perform-in-actor dispatcher
      (dcase* whole-msg
        ...
        (actors/internal-message/security:srp-node-id-rsa (node-id)
           ...
           (inject-into-actor intf ;; with-worker ()
             (handler-bind ((error (lambda (c)
                                     ;; if any negotiation errors we shut down immediately
                                     (declare (ignore c))
  ...
  (with-slots (title
    ...
    (inject-into-actor intf ;; for =cont
      (flet
        ...
        #+:USING-ECC-CRYPTO
        (setf srp-ph2-begin-ecc (=contx #'start-phase2-ecc))
        #-:USING-ECC-CRYPTO
        (setf srp-ph2-begin-rsa (=contx #'start-phase2-rsa))
        
        (setf srp-ph2-reply     (=contx #'phase2-reply)
              srp-ph3-begin     (=contx #'start-phase3)
...
(defun tst ()
  (let (ans)
    (log-info :SYSTEM-LOG "TIMING: ~A :VALUE ~A"
              ...
              ans)))
(tst)
(stac:spawn 'tst) ;; no go...
(stac:spawn-worker 'tst)
