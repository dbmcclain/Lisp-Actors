...
(defun link-to (proc)
  (pushnew proc (process-links self)))

(defun unlink-from (proc)
  (deletef (process-links self) proc))
     ...
     (:exit (from reason)
      (lambda ()
        (flet ((kill-and-propagate ()
                 ;; (stactors/executives:nullify self)
                 (let ((links (shiftf (process-links self) nil)))
                   (dolist (proc links)
...
(defun link (proc)
  (link-between self proc))

(defun unlink (proc)
  (unlink-between self proc))

;; --------------------------------------------------

(defun exit (reason &optional (proc self))
  (send proc :exit self reason))

(defun trap-exits (t/f)
  (setf (trapping-exits self) t/f))
