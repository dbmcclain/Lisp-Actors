  ...
  (nlet iter ((v   val)
    ...
    (if (not (plusp nb))
      ...
      (let ((bits (ldb (byte 11. 0) v)))
        (go-iter (ash v -11.) (- nb 11.) (cons (aref *wordlist* bits)
                                               lst))
        ))
    ))
...
(defun convert-wordlist-to-int (lst)
  (nlet iter ((lst lst)
    ...
    (if (endp lst)
      ...
      (go-iter (cdr lst)
               (logior (ash v 11.)
                       (wrd-binary-search (car lst))))
      )))

#|
(progn
  (assert (= 2048. (length *wordlist*)))
  (loop for wrd across *wordlist* do (wrd-binary-search wrd))
  (assert (eq :ok (handler-case
                      (wrd-binary-search "AAA")
                    (error (c)
                      :ok))))
  (assert (eq :ok (handler-case
                      (wrd-binary-search "ZZZ")
                    (error (c)
                      :ok))))
  (assert (eq :ok (handler-case
                      (wrd-binary-search "XXX")
                    (error (c)
                      :ok))))
  (assert (null (convert-int-to-wordlist 0)))
  (assert (zerop (convert-wordlist-to-int nil)))
  (let ((val (core-crypto:random-between 0 (ash 1 256.))))
