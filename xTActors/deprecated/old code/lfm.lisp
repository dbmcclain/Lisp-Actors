...
(defun normalized-tell-logger (service entry-kind msg)
  (let ((k-cont (actor (a)
                  (log-message a entry-kind msg (timestamp)
                               (or self
                                   (mp:get-current-process))))
                ))
    (find-actor k-cont service)))
  ...
  (um:dlambda
    (:log-message (entry-kind msg timestamp who-from)
     (let ((s (with-output-to-string (s)
                (funcall (lfm-printer state) s entry-kind msg timestamp who-from)))
           (stream (lfm-stream state)))
       (progn ;; with-worker ()
         (stream:with-stream-output-lock stream
           (princ s stream)
           (force-output stream)))
      ...
      (:set-stream (new-stream &optional new-needs-close)
       (let ((needed-close (lfm-needs-close state))
             (old-stream   (lfm-stream state))
...
(defun set-printer (logger new-printer)
  ...
  (send logger :new-printer new-printer))

(defun set-stream (logger new-stream &optional new-needs-close)
  (send logger :new-stream new-stream new-needs-close))

(defun log-message (logger entry-kind msg timestamp who-from)
  (send logger :log-message entry-kind msg timestamp who-from))
