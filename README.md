...
But this doesn't scale well. There is some machine limit to the number of threads that the system can handle. Attempting to distribute the code among several servers becomes a communication, coordination, and security chore. The dedicated thread may become under-utilized, as a machine resource.
...
There becomes no inherent limit to the number of concurrent parallel tasks in a running Actors system. It scales extremely well, easily accommodating millions of concurrent tasks. Actors can SEND messages to remote Actors without even realizing it. So distributed code becomes a natural thing to do. And the distribution of code can be changed on the fly without affecting any other code. Proxy Actors provide a highly secure remote communication facility with remote Actors. Security is by way of unattributable malleable encrypted communications using a double-ratchet evolving key-set between connection endpoints.
