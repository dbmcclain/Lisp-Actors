...
Assume private client key `c`, and public client key `C = c*G`. Server private key `s` and server public key `S = s*G`. (scalar numbers are lower case, ECC points are upper case. G = ECC generator point. We happend to use Curve1174, but can easily be changed.). Generate random `a` at client and start a connection by sending `(A = a*G, C)`. Server generates random `b` and sends back `(B = b*G, S)`. Now shared private key becomes `EKey = H(a*B | c*B | a*S)` at the client side, and `EKey = H(A*b | C*b | A*s)` at the server side. These two keys are the same. And only those with knowledge of their own random value and private key can produce the shared secret key.
