...
Rather than relying on Locks, we keep in mind that parallel execution of code snippets is possible at any time. And so we make use of Functional Programming techniques to avoid mutating any shared data. BECOME is used to safely change state in a manner that is safe. Two or more parallel executions that attempt a BECOME are silently coodinated - only one of them will succeed, and the others will be automatically retried with re-delivery of the messages that they previously received.
