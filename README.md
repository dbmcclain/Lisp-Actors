...
Up to now, we needed Continuations in the CALL/RETURN world to effect what we now have. Lisp doesn't provide Continuations, and for many good reasons. But in effect, we are doing just that for ourselves using this new approach. Nearly every langugae supports at least one current continuation - the stack state as it is at the moment. We don't thread-switch, but rather we save the current continuation (stack state) by leaving it intact, and make your thread join the dispatcher pool until an answer can be returned. This becomes much less wasteful of resources than traditional Continuations, which either switch-out threads or else copy large stack regions to and from memory. Or else, the language system uses spaghetti stacks and stack switching -- e.g., Smalltalk and Concurrent-ML.
