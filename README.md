...
There is a single mailbox, or Event Queue, from which all threads in the pool feed, and to which all message SENDs are sent. No thread-specific message mailbox. Messages are sent to the Actors "system" which then apportions out new messages to any available thread in the dispatch pool. Code does not belong to any thread. But all threads can execute code on demand as needed.
