...
In publicly deployed systems, as long as you only permit the operations of SEND, BECOME, and CREATE, it becomes impossible for malicious clients to hose the system. The worst they can do is SEND to SELF infinitely. But since messages are multiplexed in the Sponsors, that cannot tie up a system. All they do is waste one cycle of message dispatch. Also, they can only SEND to services whose Actor identity is made known to them. They cannot cripple the system by sending a message to arbitrary addresses. And Actors simply ignore ill-formed messages, and provide no feedback to the sender.
