...
The gradual incline in both tests at higher loads shows the increasing cost of GC. 33 Million Actors takes about 70 MB of memory (not much!), and the system is barely breaking a sweat with all 8 cores lit up to max utilization. The Lisp system remains responsive to keyboard and editing chores while the tests are running. Imagine running 33 Million live socket connections.
