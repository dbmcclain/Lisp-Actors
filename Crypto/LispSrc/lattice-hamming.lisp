...
#+nil
(defun enc-sub (m sel start end)
  #F
  (declare (integer sel)
           (fixnum start end))
  (let ((ans (copy-seq (aref m 0))))
    (declare (vector fixnum ans))
    (fill ans 0)
    (um:nlet iter ((ix start))
      (declare (fixnum ix))
      (cond ((>= ix end)
             ans)
            ((logbitp ix sel)
...
(defun enc-mat*v (m sel)
  ...
  (let* ((nrow    (length m))
         (nhd     (ash nrow -2))
         (modulus (mod-base)))
    (declare (fixnum nrow nhd modulus))
    (ac:ask
     (ac:create (lambda (cust)
                  (ac:let-β ((v1  (ac:service #'enc-sub m sel 0         nhd))
                             (v2  (ac:service #'enc-sub m sel nhd       (* 2 nhd)))
                             (v3  (ac:service #'enc-sub m sel (* 2 nhd) (* 3 nhd)))
                             (v4  (ac:service #'enc-sub m sel (* 3 nhd) nrow)))
                    (ac:send cust
                             (with-mod modulus
                               (map-into v1 #'lmod
                                         (map-into v1 #'+ v1 v2 v3 v4)))
                             )))))
    ))
...
(defun lat-encode (pkey v)
  ;; v should be a vector of octets
  ;; Encodes octet vector into a list of cyphertext vectors
  (labels ((sub (start end)
             (declare (fixnum start end))
             (loop for ix fixnum from start below end nconc
                     (lat-encode-byte pkey (aref v ix)))))
    (let ((nb  (length v)))
      (declare (fixnum nb))
      (coerce
       (cons *lattice-m*
             (cond ((>= nb 4)
                    (let ((nb/4  (ash nb -2)))
                      ...
                      (ac:ask
                       (ac:create
                        (lambda (cust)
                          (ac:let-β ((l1  (ac:service #'sub 0 nb/4))
                                     (l2  (ac:service #'sub nb/4 (* 2 nb/4)))
                                     (l3  (ac:service #'sub (* 2 nb/4) (* 3 nb/4)))
                                     (l4  (ac:service #'sub (* 3 nb/4) nb)))
                            (ac:send cust (nconc l1 l2 l3 l4))))
                        ))))
                   (t
                    (loop for x across v nconc
                            (lat-encode-byte pkey x)))
...
(defun lat-decode (skey cs)
  ...
  (let* ((nel  (length cs))
         ...
         (m    (aref cs 0)))
    (declare (fixnum nel nb m)
             (vector (unsigned-byte 8) ans))
    (assert (eql m *lattice-m*))
    (with-mod m
      (let ((bv  (make-array (1- (length cs))
                             :element-type 'bit)))
        (declare (vector bit bv))
        ...
        (loop for bpos fixnum from 0 by 8
              for ix fixnum from 0 below nb
              do
