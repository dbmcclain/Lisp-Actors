...
(defvar *ecc-a*  1)
(defvar *ecc-b*
  (big32 #x02F40E7E #x2221F295 #xDE297117 #xB7F3D62F
...
(defvar *ecc-r*
  ...
  ;; (r * gen = infinity )
  (big32 #x03FFFFFF #xFFFFFFFF #xFFFFFFFF #xFFFFFFFF
...
(defmethod ecc-infinite-p ((pt ecc-infinity))
  t)
  ...
  (um:match pt
    ...
    (#T(ecc-pt :x x :y y)
     (make-ecc-pt
      :x x
      :y (gf+ x y)))
    (#T(ecc-projective-pt :x x :y y :z z)
  ...
  (um:match pt
    ...
    (#T(ecc-pt)
       (case *nbits*
         ...
         (t  (ecc-affine-double a))
         ))
    (#T(ecc-projective-pt)
       (ecc-projective-double pt))
    ))
       
(defun ecc-basic-double (a)
  (case *nbits*
    (571  (with-fast-impl
           (c-ecc571-add a a)
           (ecc-affine-double a)))
...
(defmethod ecc-basic-add ((a ecc-infinity) b)
  b)

(defmethod ecc-basic-add (a (b ecc-infinity))
  a)

(defmethod ecc-basic-add ((a ecc-pt) (b ecc-pt))
  (case *nbits*
    (571  (with-fast-impl
           (c-ecc571-add a b)
           (ecc-affine-add a b)))
    
    (t  (ecc-affine-add a b))
    ))

(defun ecc-affine-sub (a b)
  (ecc-basic-add a (ecc-negate b)))

(defmethod ecc-sub ((a ecc-infinity) b)
  (ecc-negate b))

(defmethod ecc-sub (a (b ecc-infinity))
  a)

(defmethod ecc-sub ((a ecc-pt) (b ecc-pt))
  (case *nbits*
    (571  (with-fast-impl (c-ecc571-sub a b)
                          (ecc-affine-sub a b)))
    
    (t  (ecc-affine-sub a b))
    ))

(defun ecc+ (a &rest args)
  (reduce 'ecc-basic-add args :initial-value a))

(defun ecc- (a &rest args)
  (if args
      (reduce 'ecc-sub args :initial-value a)
    (ecc-negate a)))
...
(defun ecc-projective-infinity ()
  #|#.(make-ecc-projective-pt :x 1 :y 1 :z 0 :alpha 1)|#
  (ecc-infinity))

(defmethod ecc-infinite-p ((pt ecc-projective-pt))
  (zerop (ecc-projective-pt-z pt)))

(defmethod ecc-negate ((pt ecc-projective-pt))
  (um:bind* ((:struct-accessors ecc-projective-pt (x y z) pt))
    (make-ecc-projective-pt
     :x x
     :y (gf+ (gf* x z) y)
     :z z
...
(defmethod ecc-basic-add ((a ecc-projective-pt) (b ecc-projective-pt))
  (ecc-projective-add a b))

(defmethod ecc-sub ((a ecc-projective-pt) (b ecc-projective-pt))
  (ecc-add a (ecc-negate b)))

(defmethod ecc-projective ((pt ecc-infinity) &key alpha)
  (declare (ignore alpha))
  pt)

(defmethod ecc-affine ((pt ecc-infinity))
  pt)

(defmethod ecc-projective ((pt ecc-projective-pt) &key alpha)
  (declare (ignore alpha))
  pt)

(defmethod ecc-projective ((pt ecc-pt) &key alpha)
...
(defmethod ecc-affine ((pt ecc-pt))
  pt)

(defmethod ecc-affine ((pt ecc-projective-pt))
...
(defmethod ecc-double ((pt ecc-pt))
  (ecc-basic-double pt))

(defmethod ecc-double ((pt ecc-projective-pt))
  (ecc-projective-double pt))

(defmethod ecc-double ((pt ecc-infinity))
  pt)

(defmethod ecc-add ((pt1 ecc-pt) (pt2 ecc-pt))
  (ecc-basic-add pt1 pt2))

(defmethod ecc-add ((pt1 ecc-projective-pt) (pt2 ecc-projective-pt))
  (ecc-projective-add pt1 pt2))

(defmethod ecc-add ((pt1 ecc-infinity) pt2)
  pt2)

(defmethod ecc-add (pt1 (pt2 ecc-infinity))
  pt1)
...
(defmethod ecc-basic-mul ((pt ecc-pt) n &key (alpha 1))
  (case *nbits*
    (571  (with-fast-impl (c-ecc571-mul pt n :alpha alpha)
                          (ecc-projective-mul pt n :alpha alpha)))
    (t (ecc-projective-mul pt n :alpha alpha))))

(defmethod ecc-basic-mul ((pt ecc-projective-pt) n &key (alpha 1))
  (ecc-projective-mul pt n :alpha alpha))

(defmethod ecc-basic-mul ((pt ecc-infinity) n &key alpha)
  (declare (ignore n alpha))
  pt)
...
(defmethod ecc-pt= ((pt1 ecc-infinity) pt2)
  (ecc-infinite-p pt2))

(defmethod ecc-pt= (pt1 (pt2 ecc-infinity))
  (ecc-infinite-p pt1))

(defmethod ecc-pt= ((pt1 ecc-pt) (pt2 ecc-pt))
  (um:bind* ((:struct-accessors ecc-pt ((x1 x)
                                        (y1 y)) pt1)
             (:struct-accessors ecc-pt ((x2 x)
                                        (y2 y)) pt2))
...
(defmethod ecc-pt= ((pt1 ecc-projective-pt) (pt2 ecc-projective-pt))
  (um:bind* ((:struct-accessors ecc-projective-pt ((x1 x)
                                                   (y1 y)
                                                   (z1 z)) pt1)
             (:struct-accessors ecc-projective-pt ((x2 x)
...
(defun ecc-halve (pt)
  (um:bind* ((:struct-accessors ecc-pt (x y) pt)
             (lam (gf-solve-quadratic (gf+ x *ecc-a*)))
             (xh  (gf-sqrt (gf+ y
                                (gf* x (gf+ lam 1)))))
             (yh  (gf* xh (gf+ lam xh))))
    (make-ecc-pt
...
(defmethod ecc-solution-p ((q ecc-pt))
  ;; verify: y^2 + x*y = x^3 + a*x^2 + b
  (um:bind* ((:struct-accessors ecc-pt (x y) q))
    (zerop
     (gf+ (gf* y (gf+ x y))
          (gf* x x (gf+ x *ecc-a*))
          *ecc-b*))))

(defmethod ecc-solution-p ((q ecc-projective-pt))
  ;; (x,y) --> (x',y',z') = (s^2*x, s^3*y, s)
  ;;
  ;; verify: y^2 + x*y = x^3 + a*x^2 + b
  ;; -->  y'^2 + z'*x'*y' = x'^3 + a*z'^2*x'^2 + b*z'^6 
  (um:bind* ((:struct-accessors ecc-projective-pt (x y z) q)
             (xz  (gf* x z)))
    (zerop
     (gf+ (gf* *ecc-b* (gf^ z 6))
...
(defmethod ecc-validate-public-key ((q ecc-pt))
  (um:bind* ((:struct-accessors ecc-pt (x y) q))
    (assert (not (ecc-infinite-p q))) ;; not infinity
    (assert (<= (integer-length x) *nbits*))
    (assert (<= (integer-length y) *nbits*))
    ;; (assert (= (gf-trace x) (gf-trace *ecc-a*)))
    (assert (ecc-solution-p q))
    ...
    (assert (ecc-infinite-p (ecc-add q (ecc-basic-mul q (1- *ecc-r*)))))
    (assert (not (ecc-infinite-p (ecc-basic-mul q *ecc-h*))))
    q))

(defmethod ecc-validate-public-key ((q ecc-projective-pt))
  (um:bind* ((:struct-accessors ecc-projective-pt (x y z) q))
    (assert (not (ecc-infinite-p q))) ;; not infinity
    (assert (<= (integer-length x) *nbits*))
    (assert (<= (integer-length y) *nbits*))
    (assert (<= (integer-length z) *nbits*))
    ;; (assert (= (gf-trace (gf/ x (gf* z z))) (gf-trace *ecc-a*)))
    (assert (ecc-solution-p q))
    ;; we have to say the next one like this because the mul operator
    ;; performs a modulo *ecc-r* ahead of the point operation.
    ;; Note: this clause also catches those points with
...
(defun ecc-compress-pt (pt)
  ...
  (if (ecc-infinite-p pt)
      0
    ;; else
    (um:bind* ((:struct-accessors ecc-pt (x y) (ecc-affine pt)))
...
(defun ecc-decompress-pt (xc)
  ...
  (if (zerop xc)
    ...
    (let* ((trxor (logand xc 1))
           ...
           (xsq   (gf* x x))
           (s     (ensure-trace (gf-solve-quadratic
