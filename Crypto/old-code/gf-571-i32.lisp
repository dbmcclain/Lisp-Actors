...
(defmethod print-object ((obj gf-val) stream)
  (format stream "#<gf-val ~{#x~8,'0x~^ ~}"
          (let ((wds (gf-val-wds obj)))
            (declare (sys:simple-int32-vector wds))
            (loop for ix from 0 below 18 collect
                  (get-wrd wds ix)) )))
...
(defun make-gf-wds (&optional (val 0))
  #F
  (let ((wds (sys:make-typed-aref-vector 72)))
    (fill-wds wds val)))
...
(defun fill-wds (wds val)
  ...
  (cond
   ((integerp val)
    (loop for ix fixnum from 17 downto 0 do
   ...
   ((or (consp val)
    ...
    (let* ((nval (length val))
      ...
      (loop for ix fixnum from nz below 18
            for jx from 0
            do
...
(defun clone-gf-val (x)
  ...
  (let ((wdsx  (gf-val-wds x))
    ...
    (loop for ix from 0 below 18 do
...
(defun gf+= (gfv incr)
  ...
  (let* ((wdsa (gf-val-wds gfv))
    ...
    (loop for ix fixnum from 0 below 18 do
...
(defvar $prim
  ;; B571: t^571 + t^10 + t^5 + t^2 + 1d
  (new-gf-val (list #x08000000 #x00000000 #x00000000 #x00000000
    ...
    (multiple-value-bind (nw nbits) (truncate n 32)
      ...
      (when (plusp nw)
        (loop for ix fixnum from 0 
              for jx fixnum from nw below 18
              do
              (setf (raw-wrd wds ix) (raw-wrd wds jx)
                    (raw-wrd wds jx) 0))
        (loop for ix fixnum from (- 18 nw) below nw do
              (setf (raw-wrd wds ix) 0)) )
      ...
      (when (plusp nbits)
        (let ((v1   (get-wrd wds 0))
          ...
          (declare ((unsigned-byte 32) v1)
                   (fixnum nbc))
          (loop for ix fixnum from 0 below (- 17 nw)
...
(defun step-state= (gfv)
  ...
  (let* ((wds  (gf-val-wds gfv))
    ...
    (declare (sys:simple-int32-vector wds)
             ((unsigned-byte 32) v))
    (when (logbitp 27 v)
      (setf (get-wrd wds 0) (ldb (byte 27 0) v)
            (get-wrd wds 17) 
            (logxor (get-wrd wds 17) #x425))))
  gfv)
...
(defun gf-logbitp (index v)
  ...
  (let ((wds (gf-val-wds v)))
    ...
    (multiple-value-bind (nwd nbit) (truncate index 32)
      ...
      (logbitp nbit (get-wrd wds (- 17 nwd))) )))
