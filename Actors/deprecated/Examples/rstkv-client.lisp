...
(ac:make-remote-actor "eval@rincon.local"
                      :register :REVAL)
(ac:register-actor :LISTENER
                   (ac:make-actor (lambda (&rest msg)
                                    (ac:pr msg))))
                   
(send :reval `(send ,(ac:usti (ac:find-actor :LISTENER)) (machine-instance)))

(let ((listener (ac:make-actor (lambda (&rest msg)
                                 (ac:pr msg)))))
  (send :eval `(send ,(ac:usti listener) (machine-instance))))
