  ...
  (let ((g!block (gensym)))
    `(block ,g!block
       (let ((%sk (lambda (&rest args)
                    (return-from ,g!block (apply #'values args)))))
         (=values (progn
                    ,@body))))))
...
(defmacro with-future (args expr &body body)
  `(=bind ,args
       (blocking ,expr)
     ,@body))
|#
...
(defun do-with-blocking-future (form-fn body-cont)
  (apply body-cont (multiple-value-list
                    (exec-with-timeout *blocking-timeout*
                                       form-fn))))

(defmacro with-future (args form &body body)
  `(do-with-blocking-future
    (lambda ()
      (with-cont
        ,form))
    ,(if (member '&rest args)
  ...
  (handle-timeouts
    (with-future (&rest args)
        (progn
          (sleep 2)
          (=values 2 3 15)
          (pr "should not still in remote clause"))
      (pr args))
    :timeout 3
    :on-timeout
    (progn
