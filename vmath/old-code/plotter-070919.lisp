...
(defclass <plotter-pane> (capi:output-pane)
  ...
  ((lock      :accessor plotter-lock           :initform (mp:make-lock))
   ...
   (full-crosshair  :accessor plotter-full-crosshair :initarg :full-crosshair :initform nil)
  ...
  (:default-initargs
   ...
   :destroy-callback 'discard-backing-image
...
(defun do-with-locked-plotter-pane (pane fn)
  (mp:with-lock ((plotter-lock pane))
    (funcall fn)))
...
(defun do-with-delayed-update (pane fn)
  (let ((pane (plotter-pane-of pane))) ;; could be called with symbolic name for pane
        ...
        (let* ((top  (+ (box-top box)
          ...
          (gp:with-graphics-scale (port sf sf)
            (gp:draw-image port image left top
                           ...
                           ;; the the COCOA system bombs out really badly...
    ...
    |#
       ...
       (if backing-image
           (gp:draw-image pane backing-image 0 0
                          :from-width  (gp:image-width  backing-image)
                          :from-height (gp:image-height backing-image)
                          :to-width    (round (* sf nominal-width))
                          :to-height   (round (* sf nominal-height)))
         (progn
           (dolist (item (display-list-items pane))
             (funcall item pane pane x y width height))
           (draw-legend pane pane)
    ...
    (when dest
      (with-locked-plotter-pane pane
        #+:COCOA (with-bare-pdf-image (pane)
                   (save-pdf-plot pane (namestring dest)))
        #+:WIN32 (save-image pane (namestring dest))
      )))
