    ...
    (gp:with-graphics-state (port
      ...
      (gp:with-graphics-scale (port sf sf)
        ...
        (case symbol
          ...
          (:sampled-data
           (let ((dotfn (get-symbol-plotfn port :filled-circle
                                           border-color border-thick
                                           symbol-filled fill-color)))
             (loop for pair = (next-item pairs)
                   while pair
                   do
                   (destructure-vector (x y) pair
                    ...
                    (gp:with-graphics-state
                      ...
                      (when (and (legend-info-symbol item)
                        ...
                        (funcall (get-symbol-plotfn port (legend-info-symbol item)
                                                    ...
                                                    (legend-info-border-thick item)
                                                    (legend-info-symbol-filled item)
                                                    (legend-info-fill-color item))
                                 (+ x 18) (- y (/ effht1 2))
                                 )))
