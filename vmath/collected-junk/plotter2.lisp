...
(defun get-symbol-plotfn (port symbol)
      ...
      (gp:with-graphics-scale (port sf sf)
        ...
        (case symbol
          ...
          (t
           ...
           (when symbol
             (let ((plotfn (get-symbol-plotfn port symbol)))
               (map nil (lambda (x y)
                          (apply plotfn
                                 (multiple-value-list
...
(defun stamp-logo (pane port)
  (when *ext-logo*
    (with-image (port (logo (gp:convert-external-image pane *ext-logo*)))
      (let ((sf (plotter-sf pane)))
        (gp:with-graphics-scale (port sf sf)
          (gp:draw-image port logo 40 50))
        ))
    ))
                         ...
                         (gp:with-graphics-scale (port sf sf)
                           (gp:with-graphics-state
                             ...
                             (when fullgrid
                               (when xlog
                                 (gp:with-graphics-state
                                     (port :foreground #.(color:make-gray 0.75))
        ...
        (multiple-value-bind (txtwd txtht txtbase)
          ...
          (let* ((totwd (+ txtwd  (* sf 40)))
            ...
            (gp:with-graphics-scale (port sf sf)
              ...
              (gp:with-graphics-state (port :foreground
      ...
      #+:WIN32 (and backing-image
        ...
        (let* ((gs   (gp:get-graphics-state pane))
          ...
          (gp:with-pixmap-graphics-port
              (pm pane port-width port-height
                  :background bg
                  :foreground fg
                  :clear      t)
            (dolist (item (um:collector-contents display-list :discard nil))
              (funcall item pane pm x y width height))
            (draw-legend pane pm)
            (save-backing-image pane pm)
            (gp:draw-image pane backing-image 0 0)))
        )))
     ...
     #'(lambda (pane port xarg yarg width height)
         ...
         (let* ((x (+ offset-x (get-x-location pane x)))
           ...
           (let* ((font-attrs (gp:font-description-attributes (gp:font-description font)))
                  (font-name  (getf font-attrs :name))
                  (font-size  (getf font-attrs :size)))
             (apply #'add-label port str (* sf x) (* sf y)
                    :font        font-name
                    :font-size   font-size
                    :color       color
                    :x-alignment x-align
